...
export default function AdminBookings() {
  ...
  const assignWorker = async (b, workerId) => {
    ...
    if (!worker) {
      addToast('Worker not found', 'error');
    ...
    if (!workerPhone) {
      addToast('⚠️ Worker has no phone number. Please update worker details first.', 'warning');
  ...
  const markFinished = async (b) => {
    ...
    addToast('Work status updated.', 'success');
  };
  ...
  const resolveDispute = async (b) => {
    ...
    if (!decision) {
      addToast('Select a decision type first.', 'warning');
      return;
    }
    ...
    addToast('Dispute resolved: ' + decision, 'success');
  };
  ...
  const setPriceQuote = async (bookingId) => {
    ...
    addToast('Quote sent! User will be notified with final price: ₹' + pricing.finalTotal, 'success');
  };
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '20px' }}>
      ...
      {shown.map(b => {
        ...
        // Find assigned worker (if any)
        let assignedWorker = null;
        if (b.assignedWorkerId) {
          assignedWorker = workers.find(w => w.id === b.assignedWorkerId);
        }
        // Find driver license if assigned worker is a driver
        let driverLicenseUrl = '';
        if (assignedWorker && assignedWorker.jobs && Array.isArray(assignedWorker.jobs)) {
          const isDriver = assignedWorker.jobs.some(j => j.id === 'driver');
          if (isDriver && assignedWorker.licenseUrl) {
            driverLicenseUrl = assignedWorker.licenseUrl;
          }
        } else if (assignedWorker && assignedWorker.gigType === 'driver' && assignedWorker.licenseUrl) {
          driverLicenseUrl = assignedWorker.licenseUrl;
        }
        ...
        return (
          <div key={b.id} style={{
            ...
            {/* DRIVER LICENSE REVIEW (if assigned worker is driver) */}
            {driverLicenseUrl && (
              <div style={{ marginBottom: '12px', padding: '10px', background: '#f0fdf4', borderRadius: '8px', border: '1px solid #34d399' }}>
                <div style={{ fontSize: '12px', fontWeight: 'bold', color: '#059669', marginBottom: '8px' }}>🚗 Driver License Uploaded</div>
                <a href={driverLicenseUrl} target="_blank" rel="noreferrer">
                  <img src={driverLicenseUrl} alt="Driver License" style={{ width: '180px', height: 'auto', borderRadius: '6px', border: '1px solid #a7f3d0' }} />
                </a>
                <div style={{ fontSize: '11px', color: '#047857', marginTop: '4px' }}>Review before approving this worker for driving jobs.</div>
              </div>
            )}
