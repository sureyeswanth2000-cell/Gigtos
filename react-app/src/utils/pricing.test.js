...
describe('calculateConsumerPlatformFee', () => {
  test('uses INR 19 for bookings up to INR 500 by default', () => {
    expect(calculateConsumerPlatformFee(400)).toBe(19);
    expect(calculateConsumerPlatformFee(500)).toBe(19);
  });

  test('uses INR 29 for bookings from INR 501 to INR 1000 by default', () => {
  ...
  test('uses INR 19 plus 2 percent above INR 1000 by default', () => {
    expect(calculateConsumerPlatformFee(1200)).toBe(43);
    expect(calculateConsumerPlatformFee(2500)).toBe(69);
  });

  test('uses superadmin-configured tiered brackets', () => {
    const settings = {
      ...
      tieredFeeMid: 25,
      tieredFeeBase: 15,
      tieredFeePercentAboveMid: 3,
      gatewayFeePercent: 0,
    };
    expect(calculateConsumerPlatformFee(300, settings)).toBe(10);
    expect(calculateConsumerPlatformFee(700, settings)).toBe(25);
    expect(calculateConsumerPlatformFee(1000, settings)).toBe(45);
  });
});

describe('calculateFinalPrice', () => {
  test('calculates no-commission pricing plus 2 percent gateway fee for base amount 400', () => {
    ...
    expect(result.paymentCharge).toBe(8.38);
    expect(result.finalTotal).toBe(427.38);
    expect(result.workerReceives).toBe(400);
  });

  test('calculates no-commission pricing plus 2 percent gateway fee for base amount 800', () => {
    ...
    expect(result.paymentCharge).toBe(16.58);
    expect(result.finalTotal).toBe(845.58);
    expect(result.workerReceives).toBe(800);
  });

  test('calculates no-commission pricing plus 2 percent gateway fee for base amount 1200', () => {
    ...
    expect(result.paymentCharge).toBe(24.86);
    expect(result.finalTotal).toBe(1267.86);
    expect(result.workerReceives).toBe(1200);
  });

  test('supports superadmin flat platform fee changes', () => {
    ...
    expect(result.paymentCharge).toBe(21);
    expect(result.finalTotal).toBe(1071);
  });
  ...
  test('accepts string numbers and converts them', () => {
    const result = calculateFinalPrice('1000');
    expect(result.finalTotal).toBe(1049.58);
  });
});
...
describe('formatPriceBreakdown', () => {
  test('formats breakdown string correctly', () => {
    ...
    expect(formatted).toContain('payment gateway fee: ₹20.58');
    expect(formatted).toContain('Total: ₹1049.58');
  });
});
